document.addEventListener("DOMContentLoaded", async () => {
  ...
  const apiService = {
    ...
    async getSuggestions(url) {
      return this.makeRequest("/get_suggestions/", "POST", { url });
    },

    async fetchRelatedLinks(query) {
      return this.makeRequest(
        `/fetch_related_links?query=${encodeURIComponent(query)}`,
        "GET"
      );
    },
  };
  ...
  const uiService = {
    ...
    renderQuestion() {
      ...
      elements.prevQuestion.disabled =
        state.currentIndex === 0 || state.questions.length === 0;
      ...
      if (data.phrases && data.phrases.length > 0) {
        ...
        data.phrases.forEach((phrase) => {
          ...
          listItem.addEventListener("click", () =>
            controller.handleSuggestionClick(phrase)
          );
  ...
  const controller = {
    ...
    setupEventListeners() {
      ...
      // Question limit
      elements.saveLimitButton.addEventListener("click", () =>
        this.handleSaveQuestionLimit()
      );
      ...
      // Suggestions
      elements.fetchSuggestionsButton.addEventListener("click", () =>
    ...
    async handleSaveQuestionLimit() {
      ...
      if (isNaN(newLimit) || newLimit < 1) {
        return uiService.showAlert(
          "Please enter a valid number (1 or higher)."
        );
      }
    ...
    changeQuestionIndex(offset) {
      state.currentIndex = Math.max(
        0,
        Math.min(state.currentIndex + offset, state.questions.length - 1)
      );
      uiService.renderQuestion();
    },
  };
